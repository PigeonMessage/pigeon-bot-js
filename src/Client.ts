...
export class PigeonClient extends EventEmitter {
  ...
  async updateMemberPermissions(
    chatId: number,
    userId: number,
    permissions: Parameters<typeof this.http.updateMemberPermissions>[2]
  ): Promise<void> {
    return this.http.updateMemberPermissions(chatId, userId, permissions);
  }
  ...
  async uploadMedia(chatId: number, formData: FormData): Promise<MessageMedia> {
    return this.http.uploadMedia(chatId, formData);
  }
  ...
  connect(): void {
    ...
    this.ws.on("open", () => {
      ...
      this.sendRaw(authMsg).catch((err) => this.emit("error", err));
    });
      ...
      try {
        ...
        switch (payload.type) {
          ...
          case "message_edited": {
            ...
            this.emit(payload.type, entity, payload.data);
            break;
          }
            ...
            case "reaction_added": {
              ...
              if (data.reaction) {
                  const unified = {
                      ...
                      reaction_id: data.reaction.id,
                      created_at: data.reaction.created_at,
                  };
                  this.emit("reaction_added", unified);
              } else {
  ...
  async markAllAsReadWs(chatId: number): Promise<void> {
    await this.sendRaw({
